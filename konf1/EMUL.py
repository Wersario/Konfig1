...
class ShellEmulator2:
    ...
    def cd(self, args):
        ...
        if path == "..":
            ...
            if os.path.isdir(new_path):
                ...
                raise FileNotFoundError(f"Ошибка: '{path}' — не существует")
    ...
    def chmod(self, args):
        ...
        if not mode.isdigit() or len(mode) != 3:
            ...
            raise ValueError("Ошибка: режим должен быть числом из трех цифр, например 755.")
        ...
        try:
            ...
            return f"Права для '{filename}' изменены на {mode}."
        except Exception as e:
            print(f"Ошибка при изменении прав: {e}")
            raise ValueError(f"Ошибка при изменении прав: {e}")
    ...
    def cp(self, args):
        ...
        if not os.path.exists(source_path):
            ...
            raise FileNotFoundError(f"Ошибка: источник '{source}' не найден.")
        ...
        try:
            if os.path.isdir(source_path):
                if os.path.exists(destination_path) and not os.path.isdir(destination_path):
                    ...
                    raise FileNotFoundError(f"Ошибка: путь назначения '{destination}' существует и не является папкой.")
            ...
            return f"'{source}' скопирован в '{destination}'"
        except Exception as e:
            print(f"Ошибка при копировании: {e}")
            raise FileNotFoundError(f"Ошибка при копировании: {e}")
